defmodule Foglet.Doors.Runner do
  ...
  def handle_info(
        {_port, {:data, data}},
        %{pty_adapter: %PTYAdapter{backend: :helper} = _adapter} = state
      ) do
  ...
  defp cleanup(state) do
    ...
    _ = remove_dropfile_working_dir(state.dropfile_working_dir, state)
  ...
  # fixed filenames under a runner-owned temporary working directory.
  @sobelow_skip ["Traversal.FileModule"]
  ...
  # sobelow: this removes only the unique per-run directory created by
  # prepare_classic_dropfiles/1 under System.tmp_dir!/0.
  @sobelow_skip ["Traversal.FileModule"]
  defp remove_dropfile_working_dir(path, state) when is_binary(path) do
    case File.rm_rf(path) do
      {:ok, _removed} -> :ok
      {:error, reason, _file} -> log_cleanup_failure(state, :dropfile_working_dir_remove, reason)
    end
  rescue
    e ->
      log_cleanup_failure(state, :dropfile_working_dir_remove, e)
      :ok
  end

  defp remove_dropfile_working_dir(_path, _state), do: :ok
  ...
  defp prepare_classic_dropfiles(state) do
    ...
    case make_dropfile_working_dir(state) do
      {:ok, working_dir} ->
        case Foglet.Doors.write_dropfiles(state.manifest.dropfile_formats, attrs, working_dir) do
          {:ok, paths} ->
            manifest = %{state.manifest | working_dir: working_dir}

            {:ok,
             %{
               state
               | manifest: manifest,
                 dropfile_paths: paths,
                 dropfile_working_dir: working_dir
             }}

          {:error, reason} ->
            _ = remove_dropfile_working_dir(working_dir, state)
            {:error, reason}
        end
  ...
  # sobelow: the directory is generated under System.tmp_dir!/0 with
  # cryptographic random bytes and created with mkdir to avoid races.
  @sobelow_skip ["Traversal.FileModule"]
  defp make_dropfile_working_dir(state, attempts \\ 3)

  @sobelow_skip ["Traversal.FileModule"]
  defp make_dropfile_working_dir(state, attempts) when attempts > 0 do
    nonce = Base.url_encode64(:crypto.strong_rand_bytes(16), padding: false)
    path = Path.join(System.tmp_dir!(), "foglet-door-dropfiles-#{state.manifest.id}-#{nonce}")

    case File.mkdir(path) do
      :ok -> {:ok, path}
      {:error, :eexist} -> make_dropfile_working_dir(state, attempts - 1)
      {:error, reason} -> {:error, {:dropfile_working_dir, reason}}
    end
  end
