...
2. **Remap ALL file paths** to resolve under the workstream directory instead of `.planning/`:
   ...
   - Plan files, summaries, verification all resolve under the workstream's phase directories
3. **Validate the workstream exists:**
   ...
   ```
4. **Use the workstream's ROADMAP.md** for phase validation, plan discovery, and progress tracking — NOT the main project ROADMAP.md. Phase numbers, names, and requirements come from the workstream's own roadmap.
...
Update STATE.md for phase start (workstream's STATE.md if active, otherwise main):
```bash
...
```bash
pi-gsd-tools commit "docs(phase-{X}): complete phase execution" --files "${PLANNING_ROOT:-.planning}/ROADMAP.md" "${PLANNING_ROOT:-.planning}/STATE.md" "${PLANNING_ROOT:-.planning}/REQUIREMENTS.md" {phase_dir}/*-VERIFICATION.md
```
</step>
