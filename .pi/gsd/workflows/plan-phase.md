...
3. **Validate the workstream exists:**
   ...
   ```
4. **Use the workstream's ROADMAP.md** for phase validation (step 3), NOT the main project ROADMAP.md. Phase numbers, names, and requirements come from the workstream's own roadmap.
...
**Critical:** When `phase_found` is `false` in init data AND a workstream is active, the phase number resolves against the **workstream's** ROADMAP — not the main project ROADMAP. A phase numbered "3" in a workstream is a completely different phase than "3" in the main project.

## 2. Parse and Normalize Arguments

Extract from $ARGUMENTS: phase number (integer or decimal like `2.1`), flags (`--research`, `--skip-research`, `--gaps`, `--skip-verify`, `--prd <filepath>`, `--reviews`, `--text`, `--ws <name>`).
...
**If `phase_found` is false:** Validate phase exists in ROADMAP.md (workstream's roadmap if `--ws` is active, otherwise main ROADMAP.md). If valid, create the directory using `phase_slug` and `padded_phase` from init:
```bash
