...
Extract `--ws <name>` from $ARGUMENTS or read the pre-injected WXP context `workstream` field. See `@.pi/gsd/references/workstream-flag.md` for full spec.

**If `--ws <name>` is present (or WXP `workstream` is non-null):**

1. Set `WORKSTREAM=name` and `PLANNING_ROOT=.planning/workstreams/${name}`
2. **Remap ALL file paths** to resolve under the workstream directory instead of `.planning/`:
   - `state_path` → `.planning/workstreams/${name}/STATE.md`
   - `roadmap_path` → `.planning/workstreams/${name}/ROADMAP.md`
   - `requirements_path` → `.planning/workstreams/${name}/REQUIREMENTS.md`
   - `phase_dir` → `.planning/workstreams/${name}/phases/`
   - Context, research, verification, UAT, reviews paths all resolve under the workstream's phase directories
3. **Validate the workstream exists:**
   ```bash
   test -d ".planning/workstreams/${name}" && echo "WS_EXISTS=true" || echo "WS_EXISTS=false"
   ```
   If false: Error with available workstreams:
   ```bash
   ls .planning/workstreams/ 2>/dev/null || echo "No workstreams found"
   ```
4. **Use the workstream's ROADMAP.md** for phase validation, gray area identification, and all roadmap reads — NOT the main project ROADMAP.md. Phase numbers, names, and requirements come from the workstream's own roadmap.
5. **Shared files remain at `.planning/`**: `PROJECT.md`, `config.json`, `codebase/`, `seeds/`. These are NOT remapped.
6. **Log the remap:** Display: `Workstream: ${name} — using ${PLANNING_ROOT}/`

**If no `--ws` and no workstream in WXP context:** Flat mode (default). All paths resolve under `.planning/` as normal.

**Critical:** When `phase_found` is `false` AND a workstream is active, the phase number resolves against the **workstream's** ROADMAP — not the main project ROADMAP. A phase numbered "3" in a workstream is a completely different phase than "3" in the main project.
...
```bash
# Core project files (shared, not remapped)
cat .planning/PROJECT.md 2>/dev/null || true
# Requirements and state come from workstream if active, otherwise main project
cat "${PLANNING_ROOT:-.planning}/REQUIREMENTS.md" 2>/dev/null || true
cat "${PLANNING_ROOT:-.planning}/STATE.md" 2>/dev/null || true
```
...
# Search in workstream phases if active, otherwise main project phases
(find "${PLANNING_ROOT:-.planning}/phases" -name "*-CONTEXT.md" 2>/dev/null || true) | sort
```
...
```bash
mkdir -p "${PLANNING_ROOT:-.planning}/phases/${padded_phase}-${phase_slug}"
```
...
**Workstream note:** If a workstream is active, `phase_dir` already resolves to the workstream's phases directory (set in initialize step). No additional path remapping needed here.
...
Commit STATE.md (workstream's STATE.md if active, otherwise main):

```bash
pi-gsd-tools commit "docs(state): record phase ${PHASE} context session" --files "${PLANNING_ROOT:-.planning}/STATE.md"
```
</step>
