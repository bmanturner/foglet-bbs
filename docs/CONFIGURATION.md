...
| `FOGLET_DOOR_MANIFEST_DIR` | Optional | bundled `priv/doors/manifests` catalog | `Foglet.Doors.list_manifests/0` | Absolute directory containing operator-managed Door Games JSON manifests. When unset, Foglet reads the reviewed manifests bundled in the release, including Usurper Reborn. Set it to a blank value to disable production/operator doors, or to an absolute directory to override the bundled catalog. Restart/reload after manifest changes. See `docs/DOOR_GAMES.md`. |
