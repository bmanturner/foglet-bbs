...
| `FOGLET_DOOR_MANIFEST_DIR` | Optional | production doors disabled / unset | `Foglet.Doors.list_manifests/0` | Absolute directory containing operator-managed Door Games JSON manifests. When unset or blank, no production/operator doors are loaded. Set it before startup and restart/reload after manifest changes. See `docs/DOOR_GAMES.md`. |
