...
| `FOGLET_DOOR_MANIFEST_DIR` | Optional | production doors disabled / unset; Docker image defaults to `/data/door-manifests` | `Foglet.Doors.list_manifests/0` | Absolute directory containing operator-managed Door Games JSON manifests. When unset or blank, no production/operator doors are loaded. Set it before startup and restart/reload after manifest changes. The Docker image seeds `/data/door-manifests` with reviewed manifests, including Usurper Reborn; bind-mount or replace that directory to override the container catalog. See `docs/DOOR_GAMES.md`. |
