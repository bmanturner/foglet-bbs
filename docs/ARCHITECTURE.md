...
Phase 34 establishes the target runtime foundation for the TUI shell: screens initialize local state through `init/1`, handle normalized messages through `update/3`, and render through `render/2` using a narrow `Foglet.TUI.Context` instead of the full App struct. Screens request runtime work with explicit `Foglet.TUI.Effect` values; App interprets those effects generically for navigation, tasks, modals, session/runtime updates, publish operations, terminal sizing, and quit. Active-screen tasks use `Effect.task/2`; App resolves the current route to the owning screen key when it interprets the effect, while `Effect.task/3` remains for intentionally explicit cross-screen routing. Full production screen family migrations remain deferred to phases 35-38, with final central App screen-specific cleanup deferred to phase 39.
