...
Thread and reply creation are rejected before board-server writes whenever the board `postable_by` policy or the thread `locked` state disallows the current user, while scoped moderators and sysops can still reply to locked threads.
...
1. **Thread policy gate**: An active user can create a thread only when the target board's `postable_by` policy permits that user's role.
   ...
   - Target: Thread creation checks the persisted board policy and the posting user's active account role before calling the board server; `:members` permits any active user role, `:mods_only` permits active mods and sysops, and `:sysop_only` permits active sysops only.
...
2. **Reply policy gate**: An active user can reply only when the target board's `postable_by` policy permits that user's role.
   ...
   - Target: Reply creation checks the persisted board policy and posting user's active account role before calling the board server.
...
3. **Locked-thread reply gate**: A locked thread rejects normal member reply creation, but permits scoped moderator and sysop replies.
   ...
   - Target: Reply creation rejects locked threads before the board server is called unless the actor is a sysop or a moderator whose moderation scope includes the board; unlocked threads continue to follow the board posting policy.
   - Acceptance: Context tests prove locked-thread replies fail for regular users, succeed for sysops, succeed for moderators scoped to the board, fail for moderators outside the board scope if such narrower scopes exist, allocate no message number on rejection, and unlocking the thread restores normal policy-based reply behavior.
...
5. **Terminal rejection copy**: Terminal users receive clear posting-policy or locked-thread errors when thread or reply submission is rejected.
   ...
   - Target: New-thread and reply submission paths show a clear terminal error that distinguishes board posting policy rejection from locked-thread rejection; locked-thread rejection copy is `This thread is locked`.
   - Acceptance: TUI tests prove rejected new-thread and reply submissions keep the user in the composing flow or show an error modal without navigating as if the post succeeded, and locked-thread rejection visibly renders `This thread is locked`.
...
- Enforce `threads.locked` for normal reply creation while preserving scoped moderator and sysop bypass.
...
- Adding a new per-board moderator membership data model - this phase consumes the authorization scope model that exists when implementation starts.
...
- `:members` means any active account role may post; pending, suspended, deleted, or missing users are not active posters.
- Locked-thread bypass is limited to sysops and moderators whose authorization scope includes the board; current site-scoped moderators satisfy that condition, and future board-scoped moderation must preserve the same behavior.
...
- [ ] Any active user role can create a thread on a `:members` board.
...
- [ ] Locked-thread replies fail for regular users through the normal context reply API.
- [ ] Locked-thread replies succeed for sysops through the normal context reply API.
- [ ] Locked-thread replies succeed for moderators whose authorization scope includes the board.
...
- [ ] TUI reply rejection copy renders `This thread is locked` for locked-thread denial.
...
|---------------------|-------|-------|--------|-------|
| Goal Clarity        | 0.92  | 0.75  | met    | The outcome is explicit: deny disallowed thread/reply attempts before board-server writes, with operator locked-thread bypass. |
| Boundary Clarity    | 0.84  | 0.70  | met    | Policy enforcement, locked-thread replies, scoped operator bypass, side-effect-free rejection, and excluded adjacent posting concerns are listed. |
| Constraint Clarity  | 0.78  | 0.65  | met    | Active-user semantics, board-server allocation, context boundaries, SSH-first UI, authorization scope, and test constraints are locked. |
...
| **Ambiguity**       | 0.15  | <=0.20| met    | Weighted clarity is 0.85. |
...
| 1 | Boundary Keeper | What adjacent posting features are excluded? | No new enum values, new board-moderator membership data model, subscription enforcement, browser workflows, moderation case management, edit/delete rules, rate limits, or content filters. |
...
| 2 | Researcher | For `postable_by: :members`, should subscription/read access also be required? | No; any active user role may post on a members-postable board. |
| 2 | Boundary Keeper | Do locked threads block mods and sysops too? | No; sysops and moderators whose scope includes the board can bypass the lock through normal reply creation. |
| 2 | Failure Analyst | What exact locked-thread copy should the terminal show? | Locked-thread denial renders `This thread is locked`. |
