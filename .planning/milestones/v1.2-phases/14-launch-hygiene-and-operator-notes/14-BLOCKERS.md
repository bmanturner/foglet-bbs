...
|--------------|-------------|------------------|-----------------|-------------------|
| Phase 9 / Phase 14 | MAIL-04, MAIL-06, HYGN-02, HYGN-03 | `mix foglet.user.reset_password HANDLE` printed `https://<host>/users/reset_password/<token>`, but Foglet has no supported HTTP reset-password interaction and no TUI token-consumption path. Tests also asserted the fake `/users/reset_password/` URL output. | CLOSED BY PHASE 15. Phase 15 changed `mix foglet.user.reset_password HANDLE` to print raw reset tokens and operator-assisted SSH instructions instead of `https://<host>/users/reset_password/<token>`, added browser-free copy guards, and aligned README operator notes with raw reset-token behavior. | Codex during Phase 14 execution missed this false promise, documented it in README operator notes, and initially marked related tests as evidence instead of a blocker. |
