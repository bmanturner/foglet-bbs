...
- Board-row leading state cluster: `◆` unread (filled diamond), `◇` read (open diamond).
...
- [ ] An unread board row's leading cluster contains `◆` (filled diamond); a read board row's contains `◇` (open diamond).
...
| 2     | Boundary Keeper | Lock SCREENS.md glyph set as-is, or distinguish required vs subscribed visually? | Lock structurally as-is: `▾`/`▸` category, `◇` read, `✓` subscribed AND required, `+` available. Required vs subscribed differs in column text, not glyph. |
| 4     | Correction      | Unread atom: SCREENS.md proposes `●`; user-corrected to `◆` filled diamond.      | Unread = `◆` (filled diamond); read = `◇` (open diamond). Deviates from SCREENS.md/Phase 20 ThreadList unread atom; intentional for BoardList. |
