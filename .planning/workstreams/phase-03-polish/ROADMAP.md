...
- [x] **Phase 4: Composer & thread creation end-to-end** - `[C]` from thread list creates a new thread; `[R]` from post reader creates a reply; the broken crash-on-compose branch is removed (completed 2026-04-20)
- [x] **Phase 5: Terminal size gate** - Terminals below the agreed minimum dimensions show a "too small" message; resizing back restores the prior screen with state intact (completed 2026-04-20)
...
- [x] 05-02-PLAN.md — Same-size guard + key-swallow guard + MultiLineInput preservation regression test (locks in state safety)
**UI hint**: yes
...
| 8. Build local widget library from Raxol primitives | 0/6 | Planned | - |
...
**Requirements**: Phase-local REQ-W-01 through REQ-W-14 (REQ-W-01..13 derived from 08-VALIDATION.md, REQ-W-14 added post-audit for the `spacer()` adoption clause of the Phase 8 goal; canonical in 08-VALIDATION.md — no ROADMAP-level REQ IDs allocated because this is polish work inside the existing widget layer).
**Depends on:** Phase 7
**Plans:** 6 plans

Plans:
- [ ] 08-01-PLAN.md — Input bucket (stateless): Button, Checkbox, RadioGroup (Wave 1; REQ-W-07/08/09)
- [ ] 08-02-PLAN.md — Input bucket (stateful): TextInput, Tabs, Menu (Wave 1; REQ-W-06/10/11)
- [ ] 08-03-PLAN.md — Display + Progress: Table, Tree, Display.Progress, Progress.Spinner (Wave 1; REQ-W-02/03/04/05)
- [ ] 08-04-PLAN.md — List.SmartList (Wave 1; REQ-W-01)
