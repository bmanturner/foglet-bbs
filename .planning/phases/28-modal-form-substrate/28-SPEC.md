...
4. **Single-source-of-truth focus routing**: Keystrokes are routed only to the field at `Modal.Form`'s `focus_index`; switching focus updates the destination of the next keystroke deterministically; leaf widgets carry no parallel focus state.
   ...
   - Acceptance: A unit test asserting a `:tab :tab :char "x"` sequence on a `[text, text, text]` form (initial `focus_index: 0`) lands `"x"` in the third field's buffer, not the first, second, or any default field. A grep across `lib/foglet_bbs/tui/widgets/input/` asserts no focus-state field on `TextInput`, `RadioGroup`, or `Checkbox` structs.
...
- [ ] A `:tab :tab :char "x"` sequence on a `[text, text, text]` form (initial `focus_index: 0`) lands `"x"` in the third field's buffer; the first and second fields' buffers remain empty; no leaf input widget struct (`TextInput`, `RadioGroup`, `Checkbox`) carries a focus-state field.
