...
must_haves:
  truths:
    ...
    - "Tabbed screen width values passed into `Tabs.render/2` and `ConsoleTable` are drawable inner-frame widths, never raw terminal columns; at 64 columns the maximum framed content budget is at most 62 columns, and usually 60 with `ScreenFrame` padding."
...
<task type="auto">
  ...
  <action>
    ...
    - The `width` value passed by screens must be the inner drawable width after subtracting frame border/padding, not the raw terminal column count. For a `{64, 22}` terminal, tests should use a tab-row content budget no greater than `62`, and screen-level `ScreenFrame` callers normally use `60`.
  ...
  <acceptance_criteria>
    ...
    - `tabs_test.exs` or layout smoke includes a framed 64-column case where the rendered tab text display width is `<= 60` when called from `ScreenFrame` screens.
...
<task type="auto">
  ...
  <action>
    ...
    Concrete changes:
    - Keep `inner_width(state)` as a drawable content width helper. For `{cols, rows}`, subtract frame border and padding before passing width into widgets; do not pass raw `cols` into `Tabs.render/2`, `ConsoleTable.init/1`, or `State.build_log_table/2`.
  ...
  <acceptance_criteria>
    ...
    - Layout smoke includes 64x22 LOG, USERS, and BOARDS assertions where no rendered text line has `x + display_width(text) > 64`; table/tab content budgets used by the screen are `<= 60`.
...
<task type="auto">
  ...
  <action>
    ...
    Ensure `InvitesState.build_table/2` accepts optional `opts \\ []`, reads `:width`, and passes it to `ConsoleTable.init/1`. Treat that width as the already-subtracted drawable content width from the caller; do not subtract again inside `InvitesState`.
  </action>
  <verify>
